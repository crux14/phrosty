 ...
 */
static Bool Window_Has_Property(xcb_connection_t* dpy, xcb_window_t win, xcb_atom_t atom) {
 ...
 */
static Bool Window_Is_Viewable(xcb_connection_t* dpy, xcb_window_t win) {
    Bool ok = False;
 ...
 */
static xcb_window_t Find_Client_In_Children(xcb_connection_t* dpy, xcb_window_t win) {
 ...
 */
static xcb_window_t* Find_Roots(xcb_connection_t* dpy, xcb_window_t root, unsigned int* num) {
    ...
    *num = 0;
    ...
    prop_cookie =
        xcb_get_property(dpy, False, root, atom_virtual_root, XCB_ATOM_WINDOW, 0, 0x7fffffff);
    prop_reply = xcb_get_property_reply(dpy, prop_cookie, NULL);
    if (!prop_reply)
        return NULL;
 ...
 */
static xcb_window_t Find_Child_At_Pointer(xcb_connection_t* dpy, xcb_window_t win) {
 ...
 * root.
 */
xcb_window_t Find_Client(xcb_connection_t* dpy, xcb_window_t root, xcb_window_t subwin) {
    xcb_window_t* roots;
